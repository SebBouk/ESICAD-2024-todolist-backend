...
utilisateurRouter.get(
  "/utilisateur/getCategorie/:IdUser",
  async (req: Request, res: Response) => {
    try {
      const IdUser = req.params.IdUser;
      const UsCat = await query(
        "SELECT c.IdCategorie, c.NomCategorie FROM categorie c JOIN associer a ON c.IdCategorie = a.IdCategorie WHERE a.IdUser = ?",
        [IdUser]
      );
      res.json(UsCat);
    } catch (error) {
...
utilisateurRouter.get(
  "/utilisateur/getListes/:IdCategorie",
  async (req: Request, res: Response) => {
    try {
      console.log("Récupération des associations...");
      const listes = await query(
        "SELECT c.IdCategorie, c.NomCategorie, l.NomListe, l.IdListe FROM categorie c LEFT JOIN liste l ON l.IdCategorie = c.IdCategorie WHERE c.IdCategorie = ?",
...
utilisateurRouter.get(
  "/utilisateur/getTaches/:IdListe",
  async (req: Request, res: Response) => {
    try {
      console.log("Récupération des associations...");
      const taches = await query(
        "SELECT l.NomListe, l.IdListe, t.* FROM liste l LEFT JOIN tache t ON l.IdListe = t.IdListe WHERE l.IdListe = ?",
        [req.params.IdListe]
      );
...
utilisateurRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: "Erreur lors de la sauvegarde des données" });
    }
  }
);
...
utilisateurRouter.post(
  "/utilisateur/listes/add/:IdCategorie",
  async (req, res) => {
    const IdCategorie = req.params.IdCategorie;
    const { NomListe } = req.body;
    const IdUser = getUserIdFromToken(req);
    const listePerso = 0;
...
utilisateurRouter.get(
  "/utilisateur/getListePerso/:IdUser",
  async (req: Request, res: Response) => {
    const IdUser = req.params.IdUser;
    const listePerso = 1;
    try {
      const listes = await query(
        "SELECT * FROM liste WHERE IdUser_createurListe = ? AND listePerso = ?",
        [IdUser, listePerso]
      );
      res.json(listes);
    } catch (error) {
...
utilisateurRouter.post("/utilisateur/listesPerso/add/:IdUser", async (req, res) => {
  const { NomListe } = req.body;
  const IdUser = req.params.IdUser;
  const listePerso = 1;
  const IdCategorie = null;
  const creationDate = new Date().toISOString().slice(0, 19).replace("T", " ");
